    ...
    <M as Middleware>::Provider: PubsubClient,
{
    ...
    >(
        ...
        broadcast_filter_events_stream(self.market_aid(), tx, f).await;
    }
    ...
    pub async fn flume_market_aid_events<E: EthEvent + Clone + std::fmt::Debug + 'static>(
        ...
        flume_events_stream(self.market_aid(), tx).await;
    }
    ...
    >(
        ...
        flume_filter_events_stream(self.market_aid(), tx, filter).await;
    }
}
    ...
    <M as Middleware>::Provider: PubsubClient,
{
    ...
    let mut evt_stream = match event.subscribe().await {
        Ok(evt_stream) => {
            event!(
                Level::TRACE,
                "[broadcast_events_stream]: subscribed to event stream"
            );
            evt_stream
        }
        Err(e) => {
            event!(
                Level::ERROR,
                "[broadcast_events_stream]: failed to subscribe to event stream with error {e}"
            );
            return;
        }
    };
        ...
        match rst_event {
            Ok(new_event) => {
                if let Err(e) = tx.send(new_event).await {
                    ...
                    event!(
                        Level::TRACE,
                        "[broadcast_events_stream]: successfully forwarded event to channel"
                    );
                }
            }
            Err(e) => event!(
                Level::WARN,
                "[broadcast_events_stream]: new event generated error {e}"
            ), // are these fatal???
        }
    }
    ...
    <M as Middleware>::Provider: PubsubClient,
{
    ...
    let mut evt_stream = match event.subscribe().await {
        Ok(evt_stream) => {
            event!(
                Level::TRACE,
                "[broadcast_filter_events_stream]: subscribed to event stream"
            );
            evt_stream
        }
        Err(e) => {
        ...
        match rst_event {
            Ok(new_event) => {
                if let Some(tfm) = filter(new_event.clone()) {
                    ...
                    event!(
                        Level::TRACE,
                        "[broadcast_filter_events_stream]: received event that didn't match filter"
                    );
                }
            }
            Err(e) => event!(
                Level::WARN,
                "[broadcast_filter_events_stream]: new event generated error {e}"
            ), // are these fatal???
        }
    }
    event!(
        Level::WARN,
        "[broadcast_filter_events_stream]: event stream ended"
    );
}
    ...
    <M as Middleware>::Provider: PubsubClient,
{
    ...
    let mut evt_stream = match event.subscribe().await {
        Ok(evt_stream) => {
            event!(
                Level::TRACE,
                "[flume_events_stream]: subscribed to event stream"
            );
            evt_stream
        }
        Err(e) => {
            event!(
                Level::ERROR,
                "[flume_events_stream]: failed to subscribe to event stream with error {e}"
            );
            return;
        }
    };
        ...
        match rst_event {
            Ok(new_event) => {
                if let Err(e) = tx.send(new_event) {
                    ...
                    event!(
                        Level::TRACE,
                        "[flume_events_stream]: successfully forwarded event to channel"
                    );
                }
            }
            Err(e) => event!(
                Level::WARN,
                "[flume_events_stream]: new event generated error {e}"
            ), // are these fatal???
        }
    }
    ...
    <M as Middleware>::Provider: PubsubClient,
{
    ...
    let mut evt_stream = match event.subscribe().await {
        Ok(evt_stream) => {
            event!(
                Level::TRACE,
                "[flume_filter_events_stream]: subscribed to event stream"
            );
            evt_stream
        }
        Err(e) => {
            event!(
                Level::ERROR,
                "[flume_filter_events_stream]: failed to subscribe to event stream with error {e}"
            );
            return;
        }
    };
            ...
            Ok(new_event) => {
                if let Some(tfm) = filter(new_event.clone()) {
                    if let Err(e) = tx.send(tfm) {
                        ...
                        event!(
                            Level::TRACE,
                            "[flume_filter_events_stream]: successfully forwarded event to channel"
                        );
                    }
                } else {
                    // we don't need to forward anything to the channel
                    event!(
                        Level::TRACE,
                        "[flume_filter_events_stream]: received event that didn't match filter"
                    );
                }
            }
            Err(e) => event!(
                Level::WARN,
                "[flume_filter_events_stream]: new event generated error {e}"
            ), // are these fatal???
        }
    }
    event!(
        Level::WARN,
        "[flume_filter_events_stream]: event stream ended"
    );
}
